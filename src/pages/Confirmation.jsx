...
export default function Confirmation() {
  ...
  return (
    <div className="container" style={{ padding: '90px 24px', maxWidth: 620, textAlign: 'center' }}>
      ...
      <h1 className="serif" style={{ fontSize: 44 }}>Thank you</h1>
      <p style={{ color: 'var(--sub)', fontSize: 17, marginTop: 10 }}>
        Your order <strong style={{ color: 'var(--gold-soft)' }}>{order.orderNumber}</strong> has been received.
      </p>

      <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: 28, margin: '32px 0', textAlign: 'left' }}>
        <div className="foot-row"><span style={{ color: 'var(--sub)' }}>Order Number</span><span>{order.orderNumber}</span></div>
        ...
        {order.method === 'cod' && (
          <p style={{ color: 'var(--sub)', fontSize: 14, marginTop: 14 }}>
            You'll pay in cash when your candles are delivered.
          </p>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
        ...
        <a href="https://wa.me/201147397783" className="btn btn-ghost">WhatsApp Us</a>
      </div>
    </div>
  )
}
